...
const VehicleTypeDetailsPage = ({ params }: VehicleTypeDetailsPageProps) => {
  ...
  if (error) return <p className="text-red-500">Error al cargar detalles del tipo de vehículo: {error}</p>;
    ...
    <>
      ...
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <p><strong>Tipo de Neumáticos:</strong> {vehicleType.tipo_neumaticos}</p>
          <p><strong>Capacidad de Carga:</strong> {vehicleType.capacidad_carga}</p>
          <p><strong>Cantidad de Conductores:</strong> {vehicleType.cantidad_conductores}</p>
          <p><strong>Ciclo de Mantenimiento (km):</strong> {vehicleType.ciclo_mantenimiento_km}</p>
          <p><strong>Es Eléctrico:</strong> {vehicleType.es_electrico ? 'Sí' : 'No'}</p>

          {vehicleType.es_electrico ? (
            <>
              <p><strong>Cantidad de Baterías:</strong> {vehicleType.cantidad_baterias ?? 'N/A'}</p>
              <p><strong>Tipo de Batería:</strong> {vehicleType.tipo_bateria ?? 'N/A'}</p>
              <p><strong>Amperaje:</strong> {vehicleType.amperage ?? 'N/A'}</p>
              <p><strong>Voltaje:</strong> {vehicleType.voltage ?? 'N/A'}</p>
            </>
          ) : (
            <>
              <p><strong>Tipo de Combustible:</strong> {vehicleType.tipo_combustible ?? 'N/A'}</p>
              <p><strong>Capacidad del Tanque:</strong> {vehicleType.capacidad_tanque ?? 'N/A'}</p>
