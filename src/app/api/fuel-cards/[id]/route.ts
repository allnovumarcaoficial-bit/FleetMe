...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (isNaN(id)) {
      return NextResponse.json({ error: 'Invalid fuel card ID' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch fuel card' }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to update fuel card', details: error.message }, { status: 500 });
  }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ message: 'Fuel card deleted successfully' }, { status: 200 });
  } catch (error: any) {
    console.error('Error deleting fuel card:', error);
    if (error.code === 'P2025') { // Record to delete does not exist
      return NextResponse.json({ error: 'Fuel card not found' }, { status: 404 });
    }
    return NextResponse.json({ error: 'Failed to delete fuel card', details: error.message }, { status: 500 });
  }
}
