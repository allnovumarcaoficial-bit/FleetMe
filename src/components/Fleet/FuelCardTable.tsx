...
const FuelCardTable = () => {
  ...
  const fetchFuelCards = useCallback(async () => {
    ...
    try {
      ...
      setFormStatus({ type: 'error', message: err.message || 'Error al cargar tarjetas de combustible.' });
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('¿Estás seguro de que quieres eliminar esta tarjeta de combustible?')) {
      return;
    }
    setLoading(true);
    setFormStatus({ type: '', message: '' });
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Error al eliminar la tarjeta de combustible.');
      }
      setFormStatus({ type: 'success', message: 'Tarjeta de combustible eliminada exitosamente.' });
      fetchFuelCards(); // Re-fetch data after deletion
    } catch (err: any) {
      setFormStatus({ type: 'error', message: err.message || 'Ocurrió un error al eliminar la tarjeta de combustible.' });
    } finally {
      setLoading(false);
    }
  };
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-none bg-[#F7F9FC] dark:bg-dark-2 [&>th]:py-4 [&>th]:text-base [&>th]:text-dark [&>th]:dark:text-white">
                <TableHead className="min-w-[155px] xl:pl-7.5 cursor-pointer" onClick={() => handleSort('numeroDeTarjeta')}>
                  Número de Tarjeta {sortBy === 'numeroDeTarjeta' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('tipoDeTarjeta')}>
                  Tipo de Tarjeta {sortBy === 'tipoDeTarjeta' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('tipoDeCombustible')}>
                  Tipo de Combustible {sortBy === 'tipoDeCombustible' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('precioCombustible')}>
                  Precio del Combustible {sortBy === 'precioCombustible' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('moneda')}>
                  Moneda {sortBy === 'moneda' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('fechaVencimiento')}>
                  Fecha de Vencimiento {sortBy === 'fechaVencimiento' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('esReservorio')}>
                  Es Reservorio {sortBy === 'esReservorio' && (sortOrder === 'asc' ? '▲' : '▼')}
                </TableHead>
            ...
            <TableBody>
              {fuelCards.map((fuelCard) => (
                <TableRow key={fuelCard.id} className="border-[#eee] dark:border-dark-3">
                  <TableCell className="min-w-[155px] xl:pl-7.5">
                    <h5 className="text-dark dark:text-white">{fuelCard.numeroDeTarjeta}</h5>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <p className="text-dark dark:text-white">
                      {fuelCard.fechaVencimiento ? new Date(fuelCard.fechaVencimiento).toLocaleDateString() : 'N/A'}
                    </p>
                  </TableCell>
                  <TableCell>
                    <p className="text-dark dark:text-white">
                  ...
                  <TableCell className="xl:pr-7.5">
                    <div className="flex items-center justify-end gap-x-3.5">
                      <Link href={`/fleet/fuel-cards/${fuelCard.id}`} className="hover:text-primary">
                        <span className="sr-only">Ver Tarjeta de Combustible</span>
                        <PreviewIcon />
                      </Link>
                      <Link href={`/fleet/fuel-cards/${fuelCard.id}/edit`} className="hover:text-primary">
                        <span className="sr-only">Editar Tarjeta de Combustible</span>
                        <PencilSquareIcon />
                      </Link>
                      <button onClick={() => handleDelete(fuelCard.id)} className="hover:text-primary">
                        <span className="sr-only">Eliminar Tarjeta de Combustible</span>
                        <TrashIcon />
                      </button>
                    </div>
                  </TableCell>
