...
const MantenimientoForm = ({ initialData, onSuccess, onCancel, selectedVehicleId }: MantenimientoFormProps) => {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        ...
        setFormStatus({ type: 'error', message: 'Error al cargar la lista de vehículos.' });
        setLoading(false);
      }
    };
    fetchVehicles();
  ...
  const validateField = (name: string, value: any): string => {
    ...
    switch (name) {
      ...
      case 'fecha':
        ...
        break;
      case 'costo':
        if (value === null || value === undefined || isNaN(parseFloat(value))) error = 'Costo inválido.';
  ...
  const validatePieceField = (piece: Piece, index: number): Record<string, string> => {
    let pieceErrors: Record<string, string> = {};
    if (!piece.name.trim()) {
    ...
    if (piece.cambio_de_pieza) {
      if (!piece.numero_serie_anterior?.trim()) {
        pieceErrors[`numero_serie_anterior-${index}`] = 'Número de serie anterior es requerido.';
      }
      if (!piece.numero_serie_nueva?.trim()) {
        pieceErrors[`numero_serie_nueva-${index}`] = 'Número de serie nueva es requerido.';
      }
    }
    return pieceErrors;
  };
  ...
  const handlePieceChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value, type } = e.target;
    const updatedPieces = [...(formData.lista_de_piezas as Piece[])];
    if (type === 'checkbox') {
      updatedPieces[index] = { ...updatedPieces[index], [name]: (e.target as HTMLInputElement).checked };
    ...
    const anyPieceChanged = newListaDePiezas.some(piece => piece.cambio_de_pieza);
  ...
  const addPiece = () => {
    setFormData(prev => ({
      ...prev,
      lista_de_piezas: [...(prev.lista_de_piezas as Piece[]), { id: Date.now(), name: '', cambio_de_pieza: false, numero_serie_anterior: '', numero_serie_nueva: '' }],
    }));
  };
  ...
  const removePiece = (id: number) => {
    setFormData(prev => ({
      ...prev,
      lista_de_piezas: (prev.lista_de_piezas as Piece[]).filter(piece => piece.id !== id),
    }));
  ...
  const validateForm = () => {
    ...
    for (const key in formData) {
      if (key !== 'lista_de_piezas') { // Exclude pieces for now
    ...
    // Validate pieces
    if (!formData.lista_de_piezas || (formData.lista_de_piezas as Piece[]).length === 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = initialData ? `/api/mantenimientos/${initialData.id}` : '/api/mantenimientos';

      const payload = {
        ...formData,
        fecha: formData.fecha?.toISOString(),
        // Map pieces to the format expected by the API (without the temporary 'id' field)
        lista_de_piezas: (formData.lista_de_piezas as Piece[]).map(({ id, ...rest }) => rest),
      ...
      setFormStatus({ type: 'success', message: `Mantenimiento ${initialData ? 'actualizado' : 'creado'} exitosamente.` });
    ...
    <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <div>
            <label htmlFor="tipo" className="mb-3 block text-body-sm font-medium text-dark dark:text-white">
          ...
          <div className="md:col-span-2 mt-4">
            <h4 className="font-medium text-dark dark:text-white mb-3">Piezas</h4>
            {(formData.lista_de_piezas as Piece[]).map((piece, index) => (
              <div key={piece.id} className="border border-stroke dark:border-dark-3 p-4 rounded-md mb-4">
                <div className="flex justify-between items-center mb-3">
                  <h5 className="font-medium text-dark dark:text-white">Pieza #{index + 1}</h5>
                ...
                />
                {errors[`pieceName-${index}`] && <p className="text-red-500 text-sm mt-1">{errors[`pieceName-${index}`]}</p>}
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  />
                  <label htmlFor={`cambio_de_pieza-${piece.id}`} className="text-dark dark:text-white">¿Hubo cambio de pieza?</label>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-3">
                    <div>
                      <InputGroup
                        ...
                        handleChange={(e) => handlePieceChange(index, e)}
                      />
                      {errors[`numero_serie_anterior-${index}`] && <p className="text-red-500 text-sm mt-1">{errors[`numero_serie_anterior-${index}`]}</p>}
                    </div>
                    <div>
                      <InputGroup
                        ...
                        handleChange={(e) => handlePieceChange(index, e)}
                      />
                      {errors[`numero_serie_nueva-${index}`] && <p className="text-red-500 text-sm mt-1">{errors[`numero_serie_nueva-${index}`]}</p>}
                    </div>
                  </div>
                )}
              </div>
            ...
            <button
              type="button"
              onClick={addPiece}
              className="inline-flex items-center justify-center rounded-md bg-blue-500 py-2 px-4 text-center font-medium text-white hover:bg-opacity-90"
            >
              <PlusIcon className="mr-2" /> Añadir Pieza
            </button>
            {errors.lista_de_piezas && <p className="text-red-500 text-sm mt-1">{errors.lista_de_piezas}</p>}
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-end gap-4">
          ...
          >
            {loading ? 'Guardando...' : (initialData ? 'Actualizar Mantenimiento' : 'Crear Mantenimiento')}
          </button>
        </div>
      </form>
    </div>
