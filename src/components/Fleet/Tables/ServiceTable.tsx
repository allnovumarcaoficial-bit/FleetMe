...
const ServiceTable = ({}: ServiceTableProps) => {
  ...
  const exportToExcel = () => {
    ...
    const datosFormateados = services.map((item) => ({
      'Nombre de Servicio': item.tipoServicio,
      Descripción: item.descripcion,
      'Fecha de Creación': new Date(item.fecha || '').toLocaleDateString(
        'es-ES'
      ),
      Estado: item.estado,
      'Odómetro Inicial': item.odometroInicial,
      'Odómetro final': item.odometroFinal,
