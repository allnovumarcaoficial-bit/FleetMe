...
const ReservorioTable = () => {
  ...
  const [reservorios, setreservorio] = useState<Reservorio[]>([]);
  ...
  const fetchreservorio = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reservorio?${params.toString()}`);
      if (!res.ok) {
        throw new Error('Failed to fetch reservorio');
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/reservorio/${id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
                ...
                >
                  ...
                  <TableCell className="xl:pr-7.5">
                    <div className="flex items-center justify-end gap-x-3.5">
                      <Link
                        href={`/fleet/reservorio/${reservorio.id}`}
                      ...
                      <Link
                        href={`/fleet/reservorio/${reservorio.id}/edit`}
                        className="hover:text-primary"
                      >
                        <span className="sr-only">
                      ...
                      <button
                        onClick={() => handleDelete(reservorio.id)}
                        className="hover:text-primary"
                      >
                        <span className="sr-only">
