...
const VehiclesTable = () => {
  ...
  const vehicleColumns: ColumnFilter[] = [
    ...
    { key: 'fecha_vencimiento_licencia_operativa', title: 'Vencimiento Licencia Operativa', type: 'dateRange' },
    { key: 'fecha_vencimiento_circulacion', title: 'Vencimiento Circulación', type: 'dateRange' },
    { key: 'fecha_vencimiento_somaton', title: 'Vencimiento Somatón', type: 'dateRange' },
        ...
        for (const key in activeFilters.columnFilters) {
          ...
          if (value !== undefined && value !== null && value !== '') {
            if (Array.isArray(value)) {
              ...
              } else if (key === 'fecha_vencimiento_licencia_operativa' && value[0] && value[1]) {
                const [startDate, endDate] = value as [Dayjs, Dayjs];
                params.append('fechaVencimientoLicenciaOperativaDesde', startDate.toISOString());
                params.append('fechaVencimientoLicenciaOperativaHasta', endDate.toISOString());
              } else if (key === 'fecha_vencimiento_circulacion' && value[0] && value[1]) {
                const [startDate, endDate] = value as [Dayjs, Dayjs];
                params.append('fechaVencimientoCirculacionDesde', startDate.toISOString());
                params.append('fechaVencimientoCirculacionHasta', endDate.toISOString());
              } else if (key === 'fecha_vencimiento_somaton' && value[0] && value[1]) {
                const [startDate, endDate] = value as [Dayjs, Dayjs];
                params.append('fechaVencimientoSomatonDesde', startDate.toISOString());
                params.append('fechaVencimientoSomatonHasta', endDate.toISOString());
