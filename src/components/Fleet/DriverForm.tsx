...
const DriverForm = ({ initialData, onSuccess, onCancel }: DriverFormProps) => {
  ...
  const [formData, setFormData] = useState<Partial<Driver>>(initialData ? {
    ...initialData,
    fecha_vencimiento_licencia: initialData.fecha_vencimiento_licencia ? new Date(initialData.fecha_vencimiento_licencia) : null,
  ...
  useEffect(() => {
    const fetchDependencies = async () => {
      try {
        ...
        const availableVehicles = vehiclesData.data.filter((v: Vehicle) => 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = initialData ? `/api/drivers/${initialData.id}` : '/api/drivers';
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...formData,
          fecha_vencimiento_licencia: formData.fecha_vencimiento_licencia?.toISOString(),
        }),
      });
      ...
      setFormStatus({ type: 'success', message: `Conductor ${initialData ? 'actualizado' : 'creado'} exitosamente.` });
    ...
    <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <div>
            <InputGroup
              ...
              value={formData.fecha_vencimiento_licencia ? formData.fecha_vencimiento_licencia.toISOString().split('T')[0] : ''}
              handleChange={handleChange}
            />
            {errors.fecha_vencimiento_licencia && <p className="text-red-500 text-sm mt-1">{errors.fecha_vencimiento_licencia}</p>}
          </div>
        ...
        <div className="mt-6 flex justify-end gap-4">
          ...
          >
            {loading ? 'Guardando...' : (initialData ? 'Actualizar Conductor' : 'Crear Conductor')}
          </button>
        </div>
      </form>
    </div>
