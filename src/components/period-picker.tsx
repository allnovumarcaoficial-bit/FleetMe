...
export function PeriodPicker<TItem extends string>({
  ...
  // Sincronizar el valor seleccionado con los cambios en defaultValue
  useEffect(() => {
    if (defaultValue) {
  ...
  return (
    <Dropdown isOpen={isOpen} setIsOpen={setIsOpen}>
      ...
      <DropdownContent
        align="end"
        className="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 max-h-60 min-w-[7rem] overflow-y-auto rounded-lg border border-[#E8E8E8] bg-white p-1 font-medium text-dark-5 shadow-md dark:border-dark-3 dark:bg-dark-2 dark:text-current"
      >
        <ul>
